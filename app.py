...
def add_copy_button(text, button_text):
    ...
    """, unsafe_allow_html=True)
# داخل قسم chicken_profits بعد حقول الإدخال
st.subheader(get_help_message("year_help", language))
year = st.radio(
    get_error_message("year_selection", language),
    [texts[language]["first_year"], texts[language]["second_year"]],
    horizontal=True,
    help=get_help_message("year_help", language)
)

# تعديل الحسابات بناء على السنة المختارة
if year == texts[language]["first_year"]:
    egg_production_rate = 0.9  # 90% إنتاج
    rental_rate = 0
else:
    egg_production_rate = 0.7  # 70% إنتاج
    rental_rate = 6 if eggs_value >= 260 else 0

total_egg_price = eggs_value * egg_production_rate * float(new_egg_price)
total_feed_cost = (days_value * 2) * float(new_feed_price)
total_rent = rental_rate

# في قسم النتائج
results_text += f"""
║ {texts[language]['selected_year']}: {year}
║ {texts[language]['production_rate']}: {egg_production_rate*100}%"""

# في قسم النصوص
texts = {
    # ... النصوص السابقة
    "first_year": {
        "العربية": "السنة الأولى",
        "English": "First Year",
        "Română": "Anul 1"
    },
    "second_year": {
        "العربية": "السنة الثانية", 
        "English": "Second Year",
        "Română": "Anul 2"
    },
    "selected_year": {
        "العربية": "السنة المختارة",
        "English": "Selected Year",
        "Română": "Anul Selectat"
    },
    "production_rate": {
        "العربية": "معدل الإنتاج",
